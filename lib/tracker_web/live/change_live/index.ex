defmodule TrackerWeb.ChangeLive.Index do
  ...
  def render(assigns) do
    ...
    <.table id="changes" rows={@streams.changes}>
      <:col :let={{_id, change}} label="#">
        <.link navigate={~p"/changes/#{change.number}"}>{change.number}</.link>
      </:col>
      <:col :let={{_id, change}} label="Title">
      ...
      <:col :let={{_id, change}} label="Author">
        {change.author}
      </:col>
      <:col :let={{_id, change}} label="Base">
        {change.base_ref}
      </:col>
      <:col :let={{_id, change}} label="Merged">
  ...
  def handle_params(params, _url, socket) do
    ...
    socket = assign(socket, :page_title, "Changes")
  ...
  def handle_event("search", %{"search" => search}, socket) do
    socket =
      ...
      |> assign(:offset, 0)
      |> load_changes()
      |> push_event("update-url", %{path: changes_path(search, 1)})
  ...
  def handle_event("next-page", _params, socket) do
    {:noreply,
     push_patch(socket, to: changes_path(socket.assigns.search, socket.assigns.current_page + 1))}
  end
  ...
  def handle_event("prev-page", _params, socket) do
    {:noreply,
     push_patch(socket,
       to: changes_path(socket.assigns.search, max(socket.assigns.current_page - 1, 1))
     )}
  end

  defp changes_path(search, page) do
    params =
      %{}
  ...
  defp load_changes(socket) do
    page =
      Tracker.Nixpkgs.Change.list!(socket.assigns.search,
